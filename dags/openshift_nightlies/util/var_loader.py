...
def get_latest_release_from_stream(base_url, release_stream):
    ...
    return {
        "openshift_client_location": f"{latest_accepted_release_url}/openshift-client-linux-{latest_accepted_release}.tar.gz",  # noqa
        "openshift_install_binary_url": f"{latest_accepted_release_url}/openshift-install-linux-{latest_accepted_release}.tar.gz",  # noqa
    }


def get_elastic_url():
    elasticsearch_config = Variable.get("elasticsearch_config", deserialize_json=True)
    return f"http://{elasticsearch_config['username']}:{elasticsearch_config['password']}@{elasticsearch_config['url']}"  # noqa
