...
class OpenshiftInstaller():
    ...
    def _get_task(self, operation="install", trigger_rule="all_success"):
        ...
        with open(f"/tmp/{operation}_task.json", 'w') as json_file:
            json.dump(config, json_file, sort_keys=True, indent=4)

        return BashOperator(
            task_id=f"{operation}",
            depends_on_past=False,
            bash_command=f"/opt/airflow/dags/repo/dags/openshift_nightlies/scripts/install_cluster.sh -p {self.platform} -v {self.version} -j {json.dumps(config)} -o {operation}",
            retries=3,
            dag=self.dag,
