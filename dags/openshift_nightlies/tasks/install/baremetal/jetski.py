...
class BaremetalOpenshiftInstaller:
    def __init__(self, dag, version, release_stream, platform, profile, build):
        ...
        self.install_secrets = Variable.get(f"baremetal_openshift_install_config", deserialize_json=True)
    ...
    def _get_webfuse_installer(self):
        return webfuse.BaremetalWebfuseInstaller(
            self.dag, self.version, self.openshift_release, self.platform, self.profile, self.openshift_build
        )
    ...
    def _get_task(self, operation="install", trigger_rule="all_success"):
        ...
        config = {**self.vars, **self.install_secrets, **{"es_server": var_loader.get_elastic_url()}}
        ...
        with open(
            f"/tmp/{self.version}-{self.platform}-{self.profile}-{operation}-task.json", "w"
        ) as json_file:
