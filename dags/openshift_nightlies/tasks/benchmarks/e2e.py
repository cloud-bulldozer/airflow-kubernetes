...
class E2EBenchmarks:
    ...
    def get_benchmarks(self):
        ...
        with TaskGroup("Index Results", prefix_group_id=False, dag=self.dag) as post_steps:  # noqa
            indexers = self._add_indexers(benchmarks)  # noqa
        return benchmarks
    ...
    def _get_benchmarks(self, benchmarks):
        for index, benchmark in enumerate(benchmarks):
            if "benchmarks" not in benchmark:
                ...
                with TaskGroup(benchmark["group"], prefix_group_id=False, dag=self.dag) as task_group:  # noqa
    ...
    def _get_benchmark(self, benchmark):
        ...
        return BashOperator(
            ...
            bash_command=f"{constants.root_dag_dir}/scripts/run_benchmark.sh -w {benchmark['workload']} -c {benchmark['command']} ",  # noqa
            retries=3,
            dag=self.dag,
            env=env,
