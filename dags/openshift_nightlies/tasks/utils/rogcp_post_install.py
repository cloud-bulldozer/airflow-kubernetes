...
class Diagnosis():
    ...
    def _get_rogcp_postinstallation(self, operation="postinstall", trigger_rule="all_success"):
        return BashOperator(
            task_id=f"{operation}_rogcp",
            depends_on_past=False,
            bash_command=f"python {constants.root_dag_dir}/scripts/utils/rogcp_post_install.py --jsonfile /tmp/{self.release_name}-postinstall-task.json --kubeconfig /home/airflow/auth/config",
            retries=3,
            dag=self.dag,
