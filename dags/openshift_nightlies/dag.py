...
class AbstractOpenshiftNightlyDAG(ABC):
    ...
    def _get_scale_ci_diagnosis(self):
        return scale_ci_diagnosis.Diagnosis(
            self.dag,
            self.version,
            self.release_stream,
            self.latest_release,
...
class CloudOpenshiftNightlyDAG(AbstractOpenshiftNightlyDAG):
    def __init__(self, version, release_stream, platform, profile, version_alias):
        ...
        self.latest_release = var_loader.get_latest_release_from_stream(
    ...
    def _get_openshift_installer(self):
        return openshift.CloudOpenshiftInstaller(
            self.dag, self.version, self.release_stream, self.latest_release, self.platform, self.profile
        )
...
class BaremetalOpenshiftNightlyDAG(AbstractOpenshiftNightlyDAG):
    ...
    def _get_openshift_installer(self):
        return jetski.BaremetalOpenshiftInstaller(
            self.dag, self.version, self.release_stream, self.platform, self.profile, self.openshift_build
        )
...
class OpenstackNightlyDAG(AbstractOpenshiftNightlyDAG):
    ...
    def _get_openshift_installer(self):
        return jetpack.OpenstackJetpackInstaller(
            self.dag, self.version, self.release_stream, self.latest_release, self.platform, self.profile
        )
...
for release in release_manifest.get_releases():
    ...
    if release["platform"] == "baremetal":
        nightly = BaremetalOpenshiftNightlyDAG(
            release["version"],
            release["releaseStream"],
            release["platform"],
            release["profile"],
            release.get("versionAlias", "none"),
            release["build"],
        )
    elif release["platform"] == "openstack":
        nightly = OpenstackNightlyDAG(
            release["version"],
            release["releaseStream"],
            release["platform"],
            release["profile"],
        ...
        nightly = CloudOpenshiftNightlyDAG(
            release["version"],
            release["releaseStream"],
            release["platform"],
            release["profile"],
